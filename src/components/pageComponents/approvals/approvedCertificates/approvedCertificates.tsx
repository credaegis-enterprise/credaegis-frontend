...
const ApprovedCertificates: React.FC<ApprovedCertificatesProps> = ({
  ...
  const getIssuedCertificates = useCallback(async () => {
    ...
    try {
      if (selectedEvent) {
        result = await myInstance.get(`/certificate/event/get/${selectedEvent}`);
        console.log(result);
      } else if (selectedCluster) {
        result = await myInstance.get(
          `/certificate/cluster/get/${selectedCluster}`
        );
      }
      else{
        router.refresh();
...
>
  ...
  ${certificate.revoked ? "text-red-400 dark:text-red-400" : 
...
>
  {
    ...
    (certificate.expiry_date && new Date(certificate.expiry_date) < new Date()) ? "Expired" : "Valid"
  }
</td>
