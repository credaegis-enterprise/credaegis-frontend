...
const [eventName, setEventName] = useState(event.event_name);
 ...
 const handleActivateEvent = async () => {
    setLoading(true);
    try {
      const response = await myInstance.put(`/member/event-control/activate/${event.event_ulid}`);
 ...
 const handleDeactivateEvent = async () => {
    setLoading(true);
    try {
      const response = await myInstance.put(`/member/event-control/deactivate/${event.event_ulid}`);
      toast.success(response.data.message);
      setIsOpen(false);
      router.refresh();
    } catch (error: any) {
  ...
  <>
          ...
          <MyButton   size="md" className="bg-green-400"
        ...
        <div className="flex flex-col justify-center w-full gap-4">
            <div className="flex gap-2 ">
                ...
                <span className="text-yellow-500 text-sm font-semibold">This name will be the new name all the certificates are registered under</span>
            </div>
        <Input type="text" label="Enter the new name" size="md" className="w-full" value={eventName}
        isInvalid={invalid}
